...
REMOTE_MAIL_PREP_CMD="true"
if [ "$DEPLOY_PROJECT_NAME" = "infra-mail" ]; then
    REMOTE_MAIL_PREP_CMD="IFACE=\$(ip route show default 2>/dev/null | awk '/default/ {print \\\$5; exit}'); [ -n \"\\\$IFACE\" ] || IFACE=eth0; MAIL_BIND_IP=\$(awk -F= '/^MAIL_BIND_IP=/{print \\\$2}' '$REMOTE_ENV_FILE' | tail -n 1); MAIL_MX2_BIND_IP=\$(awk -F= '/^MAIL_MX2_BIND_IP=/{print \\\$2}' '$REMOTE_ENV_FILE' | tail -n 1); [ -n \"\\\$MAIL_BIND_IP\" ] || { echo \"MAIL_BIND_IP ausente em $REMOTE_ENV_FILE\" >&2; exit 1; }; [ -n \"\\\$MAIL_MX2_BIND_IP\" ] || { echo \"MAIL_MX2_BIND_IP ausente em $REMOTE_ENV_FILE\" >&2; exit 1; }; [ \"\\\$MAIL_BIND_IP\" != \"\\\$MAIL_MX2_BIND_IP\" ] || { echo \"MAIL_BIND_IP e MAIL_MX2_BIND_IP nao podem ser iguais\" >&2; exit 1; }; for MAIL_IP in \"\\\$MAIL_BIND_IP\" \"\\\$MAIL_MX2_BIND_IP\"; do ip -4 addr show dev \"\\\$IFACE\" | grep -Fq \" \\\$MAIL_IP/\" || ip addr add \"\\\$MAIL_IP/24\" dev \"\\\$IFACE\"; done"
fi
REMOTE_DEPLOY_CMD="set -e && mkdir -p '$REMOTE_DIR' && cd '$REMOTE_DIR' && test -f '$DEPLOY_COMPOSE_FILE' && test -f '$REMOTE_ENV_FILE' && command -v docker >/dev/null && docker compose version >/dev/null && $REMOTE_MAIL_PREP_CMD && docker compose --env-file '$REMOTE_ENV_FILE' -f '$DEPLOY_COMPOSE_FILE' --project-name '$DEPLOY_PROJECT_NAME' config >/dev/null && docker compose --env-file '$REMOTE_ENV_FILE' -f '$DEPLOY_COMPOSE_FILE' --project-name '$DEPLOY_PROJECT_NAME' up -d --build && if [ '$DEPLOY_PROJECT_NAME' = 'infra-mail' ] && command -v rc-service >/dev/null 2>&1; then rc-service fail2ban restart >/dev/null 2>&1 || rc-service fail2ban start >/dev/null 2>&1 || true; fi && docker compose --env-file '$REMOTE_ENV_FILE' -f '$DEPLOY_COMPOSE_FILE' --project-name '$DEPLOY_PROJECT_NAME' ps"
