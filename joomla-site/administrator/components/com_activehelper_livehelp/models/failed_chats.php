...
class activehelper_livehelpModelFailed_chats extends JModelList
{
 	...
 	protected function populateState($ordering = NULL, $direction = NULL)
	{
      ...
      $query =  " Select  jls.id , CONCAT(jlu.firstname, ' ',jlu.lastname) name, jld.name domain , ".
                  " jls.username username, jls.email email, jls.phone, jls.datetime ".
                  " from  #__livehelp_sessions jls , #__livehelp_domains jld , #__livehelp_users jlu ".
                  ...
                  " and jls.id_user = jlu.id and jls.id_domain =jld.id_domain and ".
                  " jls.id not in (select jlm.session  from #__livehelp_messages  jlm ".
                  " WHERE DATE_FORMAT(jlm.datetime,'%Y%m%d') >=DATE_FORMAT(". "'" .$this->_search_start. "'" . ",'%Y%m%d')".
                  " and DATE_FORMAT(jlm.datetime,'%Y%m%d') <=DATE_FORMAT( ". "'" .$this->_search_end. "'" . ",'%Y%m%d')".
                  " group by jlm.session ) order by 5 , 2 , 3 ";
