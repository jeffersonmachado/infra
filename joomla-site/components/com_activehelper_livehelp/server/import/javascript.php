...
$database_file = $install_path . $install_directory . '/import/config_database.php';
if (file_exists($database_file)) {
        ...
        $installed_file = $install_path . $install_directory . '/import/config.php';


        if (file_exists($installed_file)) {
          $installed = true;
          include($installed_file);
...
if ($installed == true) {
  $department = db_escape_string($_REQUEST['DEPARTMENT']);
...
if ($request_id > 0) {
        ...
        if (is_array($row)) {
                ...
                $page = db_escape_string(urldecode(trim($page)));
                ...
                $query = "UPDATE " . $table_prefix . "requests SET services = '<".str_replace(",", "><", db_escape_string($_REQUEST['services'])).">'  WHERE `id` = '$request_id'";
...
if (($domain_id == 0) && (isset($_SERVER['HTTP_REFERER'])))
{
      ...
      $domain_name = db_escape_string(str_ireplace("www.", "",$domain_name));
...
?>
                ...
                if (trackingInitalized == 0)
                {
                  ...
                  if (trackerLoaded == 1) {
                    trkUrl = '<?php echo($server); ?><?php echo $install_directory; ?>/import/tracker.php?status_id=' + s_id + '&TIME=' + time + '&WIDTH=' + screen.width + '&HEIGHT=' + screen.height + '&USERID=<?php echo $_REQUEST['USERID']; ?>&services=<?php echo $_REQUEST['services']; ?>'+ '&DOMAINID=' + _vlDomain + '&AGENTID=' + _vlAgent + '&LANGUAGE=' + _vlLanguage + '&DIRECTION=' + _vldirection + '&SERVICE=' + _vlService;
...
if (_vlnot_allowed_country ==0) { 
  ...
  if ( _vlStatus_indicator  &&  _vloffline ==1 &&_custom_offline_form !='') {
      ...
      document.writeln('<img src="<?php echo($server); echo($server_directory); ?>/<?php echo($eserverName); ?>/import/status.php?service_id=' + _vlService + '&LANGUAGE=' + _vlLanguage + '&DIRECTION=' + '&DOMAINID=' + _vlDomain + '" id="LiveHelpStatus_1" name="LiveHelpStatus_1" border="0" onmouseover="s1.openInfo(this, event);" onmouseout="s1.closeInfo();"/></a>');
      _vlExternalLink =1;
   }
...
// Regular chat option for domain
   if(_vlStatus_indicator  && _vlExternalLink ==0 && _vlofftracking ==0 && _vlAgent ==0){
     ...
     document.writeln('<img src="<?php echo($server); echo($server_directory); ?>/<?php echo($eserverName); ?>/import/status.php?service_id=' + _vlService + '&LANGUAGE=' + _vlLanguage + '&DIRECTION=' + _vldirection + '&DOMAINID=' + _vlDomain + '" id="LiveHelpStatus_1" name="LiveHelpStatus_1" border="0" onmouseover="s1.openInfo(this, event);" onmouseout="s1.closeInfo();"/></a>');
   }
 else
    if(_vlStatus_indicator  && _vlExternalLink ==0 && _vlofftracking ==0 && _vlAgent !=0){     
      document.writeln('<a href="<?php echo($server);  echo($server_directory); ?>/<?php echo($eserverName); ?>/index.php" id="livechatLink" target="_blank" onclick="s1.openLiveHelp(); s1.closeInfo(); return false">');
      document.writeln('<img src="<?php echo($server); echo($server_directory); ?>/<?php echo($eserverName); ?>/import/status.php?service_id=' + _vlService + '&LANGUAGE=' + _vlLanguage +  '&DIRECTION=' + _vldirection +  '&DOMAINID=' + _vlDomain + '&AGENTID=' + _vlAgent +'" id="LiveHelpStatus_1" name="LiveHelpStatus_1" border="0" onmouseover="s1.openInfo(this, event);" onmouseout="s1.closeInfo();"/></a>');     
   } 
  }
 }
