...
$showImage = true; if (!isset($_REQUEST['USERID'])){ $_REQUEST['USERID'] = null; } else $_REQUEST['USERID'] = intval($_REQUEST['USERID']);
...
if (isset($_REQUEST['AGENTID'])){
          $agent_id = intval($_REQUEST['AGENTID']);                             
        }
...
if (!isset($_REQUEST['oUSERID'])){ $_REQUEST['oUSERID'] = null; } else $_REQUEST['oUSERID'] = intval($_REQUEST['oUSERID']);
...
if ($status_enabled == true) {
       ...
       /* Verificar si hay agentes mobiles conectados y determinar cual es el tipo de refresco que se debe usar */
        ...
        // Counts the total number of support users within each Online/Offline/BRB/Away status mode *** New Query in order to support Push notifications *** 
        $query = "SELECT DISTINCT `status`, count(`id`) FROM " . $table_prefix . "users WHERE (UNIX_TIMESTAMP(NOW()) - UNIX_TIMESTAMP(`refresh`)) < '$connection_timeout' and device_id ='' and id in (" . $users_set . ")";
        ...
        $query .= "SELECT DISTINCT `status`, count(`id`) FROM " . $table_prefix . "users WHERE (UNIX_TIMESTAMP(NOW()) - UNIX_TIMESTAMP(`refresh`)) < '$connection_timeout_mobile' and device_id <> '' and id in (" . $users_set . ")";
