...
function processForm() {
        ...
        typing(false);
		 ...
		 if("<?php echo $_REQUEST["DIRECTION"] ?>"=='1') {
			 
            top.display('<?php echo( htmlspecialchars( (string) $_REQUEST["username"], ENT_QUOTES ) ); ?>', message, '<?php echo $_REQUEST["DIRECTION"] ?>', '0');
		 }
		else {
	       top.display( message, '<?php echo( htmlspecialchars( (string) $_REQUEST["username"], ENT_QUOTES ) ); ?>' , '<?php echo $_REQUEST["DIRECTION"] ?>', '0');
		}
