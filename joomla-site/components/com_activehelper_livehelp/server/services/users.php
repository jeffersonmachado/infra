...
elseif ($action == 'Hidden' || $action == 'Offline')
{
   ...
   // new query for push notifications
   $query = "UPDATE " . $table_prefix . "users SET `refresh` = NOW(), `status` = '0' , device ='' , device_id = '' WHERE `id` = '$operator_login_id'";
   $SQL->miscquery($query);
...
if ($indicator_type == 2) {
  ...
  //error_log("SQL Chat request  pending :".$query."\n", 3, "users.log");
