 ...
 setlocale(LC_ALL, 'C.UTF-8', 'C');
    ...
    if ($layout_files_text != ''){
        ...
        for ($i = 0; $i < count($files); $i++) {
            ...
            // build website url
            if (!$files[$i]->url_home == '') {
                 if (strpos($files[$i]->url_home, 'http://') !== false or strpos($files[$i]->url_home, 'https://') !== false) {    
            ...
            if ($files[$i]->author <> ''){
                if ($files[$i]->url_author <> '') {
                    if ($mail_encode) {
                        ...
                        if (strpos($files[$i]->url_author, 'http://') !== false or strpos($files[$i]->url_author, 'https://') !== false) {    
