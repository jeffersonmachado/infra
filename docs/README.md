...
- campanhas abusivas com `From` variavel mas `Return-Path` estavel podem ser empurradas para `Spam` por um `multimap` local em [mail/rspamd/local.d/multimap.conf](/opt/results/infra/mail/rspamd/local.d/multimap.conf), usando o mapa [mail/rspamd/local.d/local_envfrom_domain_blocklist.map](/opt/results/infra/mail/rspamd/local.d/local_envfrom_domain_blocklist.map); na stack atual, esse caminho foi aplicado aos dominios `emailmkt.planoeplano.com.br`, `temosopcaoplano.com.br` e `viamaispromo.com.br` com score `8.0`, suficiente para levar os probes desses envelopes a `add header` mesmo sem depender de `DBL` ou `URIBL`
