...
: "${RIPABX_SERVER_NAME:=}"
: "${RVPN_SERVER_NAME:=}"
: "${R_OBSERVE_SERVER_NAME:=}"
: "${R_OBSERVE_CERT_DOMAIN:=}"
: "${MAIL_CERT_MDOMAINS:=}"
: "${MAIL_CERT_SERVER_NAME:=}"
: "${MAIL_CERT_SERVER_ALIASES:=}"
: "${MAIL_CERT_PRIMARY_ACME_UPSTREAM:=10.10.2.3:80}"
: "${MAIL_CERT_SECONDARY_ACME_UPSTREAM:=10.10.2.23:80}"
...
if [ -n "$R_OBSERVE_CERT_DOMAIN" ]; then
  APACHE_MDOMAIN_LINES="$APACHE_MDOMAIN_LINES
MDomain ${R_OBSERVE_CERT_DOMAIN}"
fi

if [ -n "$MAIL_CERT_MDOMAINS" ]; then
  APACHE_MDOMAIN_LINES="$APACHE_MDOMAIN_LINES
MDomain ${MAIL_CERT_MDOMAINS}"
fi

if [ -n "$ADMIN_SERVER_NAME" ]; then
  APACHE_MDOMAIN_LINES="$APACHE_MDOMAIN_LINES
MDomain ${ADMIN_SERVER_NAME}"
fi

export SERVER_NAME RESULTS_SERVER_NAME RESULTS_SERVER_ALIAS COLABORACAO_SERVER_NAME OLIMPICSHAPE_SERVER_NAME
export PLAY_SERVER_NAME PLAY_DEV_SERVER_NAME RIPABX_SERVER_NAME RVPN_SERVER_NAME R_OBSERVE_SERVER_NAME R_OBSERVE_CERT_DOMAIN ADMIN_EMAIL ACME_CA_URL HTTPS_EXTERNAL_PORT
export MAIL_CERT_MDOMAINS MAIL_CERT_SERVER_NAME MAIL_CERT_SERVER_ALIASES MAIL_CERT_PRIMARY_ACME_UPSTREAM MAIL_CERT_SECONDARY_ACME_UPSTREAM HTTPS_REDIRECT_SUFFIX UPSTREAM_HOST_30 UPSTREAM_HOST_22 UPSTREAM_HOST_7 RIPABX_INTERNAL_HOST RVPN_UPSTREAM_HOST RVPN_UPSTREAM_PORT APACHE_MDOMAIN_LINES RESULTS_ROOT_PROXY_BLOCK
export ADMIN_SERVER_NAME LDAP_BIND_PASSWORD
...
if [ -n "$MAIL_CERT_SERVER_NAME" ]; then
  templates="$templates 60-mail-cert"
fi

if [ -n "$R_OBSERVE_SERVER_NAME" ]; then
  templates="$templates 70-r-observe"
fi

if [ -n "$R_OBSERVE_CERT_DOMAIN" ] && [ -z "$R_OBSERVE_SERVER_NAME" ]; then
  templates="$templates 71-r-observe-cert-bootstrap"
fi

if [ -n "$ADMIN_SERVER_NAME" ]; then
  templates="$templates 80-admin"
fi
